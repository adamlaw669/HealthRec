  ...
  useEffect(() => {
    const fetchWeightData = async () => {
      ...
      try {
        ...
        const validEntries = response.data.filter((entry: any) => entry.weight != null)
        const labels = validEntries.map((entry: any) => new Date(entry.date).toLocaleDateString())
        const weightValues = validEntries.map((entry: any) => entry.weight)
        setChartData({
          labels,
          datasets: [{
            label: "Weight (kg)",
    ...
    datasets: [
      {
        ...
        data: [18.5, 24.9, 29.9, 30], // BMI category thresholds
        backgroundColor: [
          "rgba(34, 197, 94, 0.6)",
          "rgba(79, 70, 229, 0.6)",
          "rgba(249, 115, 22, 0.6)",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-3xl font-semibold text-gray-800 dark:text-white">Weight</h1>
            </div>
            <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
              <h2 className="text-xl font-semibold text-gray-800 dark:text-white mb-4">BMI Categories</h2>
              <Bar
                data={bmiData}
                options={{
                  responsive: true,
                ...
                {chartData ? (
                  <>
                    <li className="flex items-start">
                      ...
                      Your current weight is {chartData.datasets[0].data[chartData.datasets[0].data.length - 1]} kg.
                    </li>
                    <li className="flex items-start">
                      <span className="inline-block w-2 h-2 rounded-full bg-blue-500 mt-2 mr-2"></span>
                      Your weight has been relatively stable over this period.
                    </li>
                    <li className="flex items-start">
                      <span className="inline-block w-2 h-2 rounded-full bg-yellow-500 mt-2 mr-2"></span>
                      Regular weigh-ins help track progress towards your health goals.
                    </li>
                  </>
                ) : (
                  <li className="flex items-start">
                    <span className="inline-block w-2 h-2 rounded-full bg-gray-500 mt-2 mr-2"></span>
                    No weight data available. Connect your Google Fit account to see insights.
                  </li>
                )}
              </ul>
            </div>
