...
const HeartRateMetric = () => {
  ...
  useEffect(() => {
    const fetchHeartRateData = async () => {
      setIsLoading(true)
      try {
        const response = await fetch("http://healthrec.onrender.com/health_data", {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        })
        const result = await response.json()
        const validEntries = result.filter((entry: any) => entry.heart_rate && !isNaN(parseFloat(entry.heart_rate)))
        const labels = validEntries.map((entry: any) => new Date(entry.date).toLocaleDateString())
        const heartRateValues = validEntries.map((entry: any) => parseFloat(entry.heart_rate))
        setChartData({
          labels,
          datasets: [{
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-3xl font-semibold text-gray-800 dark:text-white">Heart Rate</h1>
            </div>
            <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <h2 className="text-xl font-semibold text-gray-800 dark:text-white mb-4">Average Heart Rate</h2>
              <div className="flex items-center justify-center">
                <div className="text-5xl font-bold text-red-500">72</div>
                <div className="ml-2 text-gray-600 dark:text-gray-400">BPM</div>
              </div>
            </div>
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              ...
              <ul className="space-y-2 text-gray-600 dark:text-gray-400">
                <li className="flex items-start">
                  ...
                  Your resting heart rate is within the normal range.
                </li>
                <li className="flex items-start">
                  <span className="inline-block w-2 h-2 rounded-full bg-blue-500 mt-2 mr-2"></span>
                  Your heart rate variability has improved by 5% this week.
                </li>
                <li className="flex items-start">
                  <span className="inline-block w-2 h-2 rounded-full bg-yellow-500 mt-2 mr-2"></span>
                  Consider more relaxation activities to reduce stress.
                </li>
              </ul>
            </div>
          </div>
